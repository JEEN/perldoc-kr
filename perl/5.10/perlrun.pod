...
Perl 프록램을 실행하는 보통의 방법은 직접실행할 수 있는 형태로 하거나, 
커맨드라인의 인수로써 소스 파일의 이름을 넘기는 것입니다.
(대화적인 Perl 환경도 있습니다 -- 어떻게 하는 가의 자세한 설명에 대해서는
L<perldebug> 를 참조해주세요)
실행을 하면, Perl 은 설정된 프로그램을 아래에 나타난 장소에서 찾습니다;
...
커맨드라인 상에서 B<-e> 가 B<-E> 의 스위치로 지정된 줄.
...
커맨드라인 상에서 최초의 파일이름으로 나타난 파일의 안.
(#! 기법을 지원하는 시스템에서는 이렇게 해 
인터프리터를 호출합니다.
L<Location of Perl> 을 참조해주세요.)
...
표준입력에서 암묵적으로 넘겨받습니다.
파일이름을 나타내는 인수가 없는 경우에만 잘 움직입니다.
STDIN 에서 읽어들이는 프로그램에 인수를 넘길 때에는 
프로그램 이름으로 명시적으로 "-" 를 설정하지 않으면 안됩니다.
